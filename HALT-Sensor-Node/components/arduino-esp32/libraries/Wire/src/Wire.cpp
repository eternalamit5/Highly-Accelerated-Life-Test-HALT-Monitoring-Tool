...
bool TwoWire::begin(int sdaPin, int sclPin, uint32_t frequency)
{
    if(sdaPin < 0) { // default param passed
        if(num == 0) {
            if(sda==-1) {
                sdaPin = SDA;    //use Default Pin // @suppress("Symbol is not resolved")
            } else {
    ...
    if(sclPin < 0) { // default param passed
        if(num == 0) {
            if(scl == -1) {
                sclPin = SCL;    // use Default pin // @suppress("Symbol is not resolved")
            } else {
    ...
    i2c = i2cInit(num, sdaPin, sclPin, _freq);
    if(!i2c) {
        return false;
    }
...
uint8_t TwoWire::endTransmission(bool sendStop)  // Assumes Wire.beginTransaction(), Wire.write()
{
    ...
    txLength = 0;
    transmitting = 0;
    //Serial.println("Twowire:return value");
    //Serial.println((last_error == I2C_ERROR_CONTINUE)?I2C_ERROR_OK:last_error);
