...
bool SDMMCFS::begin(int8_t wp, int8_t hd, int8_t miso, int8_t cs, int8_t sclk,const char * mountpoint, bool mode1bit)
{
    if(_card) {
        return true;
    }
    ...
    gpio_set_pull_mode((gpio_num_t)wp, GPIO_PULLUP_ONLY);   // CMD, needed in 4- and 1- line modes
    gpio_set_pull_mode((gpio_num_t)hd, GPIO_PULLUP_ONLY);    // D0, needed in 4- and 1-line modes
    gpio_set_pull_mode((gpio_num_t)miso, GPIO_PULLUP_ONLY);    // D1, needed in 4-line mode only
    gpio_set_pull_mode((gpio_num_t)cs, GPIO_PULLUP_ONLY);   // D2, needed in 4-line mode only
    gpio_set_pull_mode((gpio_num_t)sclk, GPIO_PULLUP_ONLY);   // D3, needed in 4- and 1-line modes
