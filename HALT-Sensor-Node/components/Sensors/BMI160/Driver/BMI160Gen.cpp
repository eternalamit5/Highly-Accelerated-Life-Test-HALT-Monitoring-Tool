...
bool BMI160GenClass::begin(SPIClass* comm_spi,const int spi_cs_pin, const int intr_pin)
{
	this->comm_spi=comm_spi;
	this->comm_i2c=NULL;
    return begin(SPI_MODE, NULL,comm_spi,spi_cs_pin, intr_pin);
}

bool BMI160GenClass::begin(Mode mode,TwoWire* comm_i2c, SPIClass* comm_spi,const int arg1, const int arg2)
{
    this->mode = mode;
    switch (this->mode) {
    case INVALID_MODE:
...
void BMI160GenClass::i2c_init()
{
  ...
  comm_i2c->beginTransmission(i2c_addr);
  if( comm_i2c->endTransmission() != 0 )
...
int BMI160GenClass::i2c_xfer(uint8_t *buf, unsigned tx_cnt, unsigned rx_cnt)
{
  ...
  if( comm_i2c->endTransmission() != 0 ) {
      Serial.println("comm_i2c->endTransmission() failed.");
  }
  if (0 < rx_cnt) {
    comm_i2c->requestFrom(i2c_addr, rx_cnt);
    p = buf;
    while ( comm_i2c->available() && 0 < rx_cnt) {
      rx_cnt--;
#ifdef DEBUG
      int t = *p++ = comm_i2c->read();
      Serial.print(" ");
      Serial.print(t, HEX);
#else
      *p++ = comm_i2c->read();;
